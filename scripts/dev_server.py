...
SYNC = 'http://127.0.0.1:8097/api/state'
class H(SimpleHTTPRequestHandler):
    ...
    def _relay(self, method):
        ...
        req = urllib.request.Request(SYNC, method=method, data=body)
        req.add_header('X-User', self._user()); req.add_header('Content-Type', 'application/json')
        if self.headers.get('If-Match'): req.add_header('If-Match', self.headers['If-Match'])
        try:
    ...
    def do_GET(self):
        ...
        if p == '/api/state': return self._relay('GET')
        return super().do_GET()
    def do_PUT(self):
        if self.path.split('?')[0] == '/api/state': return self._relay('PUT')
